...
| 7 | Memory pressure | `memory_low` | recent | crit | always | `ram_usage_mb` |
| 8 | OS out of compliance | *(deferred)* | — | info | — | — → future **release tracking** |

Defaults: `wifi_weak` −75 dBm sustained 10 min, `storage_low` <0.5 GB, `memory_low` <400 MB avail. `memory_pressure` (daily `ram_pct`, seeded **disabled**) supplies the Daily Report's RAM cutoff; `memory_low` is the live alert.

> **Charging/SoC, charging-pad, and reboot alerts removed.** `soc_low_service`,
...
> `pad_unused`, and `unexpected_reboot` were dropped. `wlc_status`/`uptime_seconds`/`boot_reason`
> telemetry and the daily pad-utilisation metric (`wlc_guest_frac`) are retained for the
...
- **#8 OS compliance** intentionally deferred to release tracking (release string, not SDK).
- **All current rules are `always`-window / fleet-wide.** The service/overnight + deployed-only window machinery remains in `EvaluateRecentAlerts` for future use, but no seeded rule uses it.
- **Removed rules** (battery-health: `battery_health_low`/`_critical`, `battery_cycles_high`, `battery_health_decline`; connectivity: `wifi_disconnects`; app/kiosk: `app_not_foreground`, `kiosk_disabled`, `app_crash`, `app_anr`; charging/SoC: `soc_low_service`, `soc_low_guest_charging`, `overnight_not_charging`, `overnight_slow_charge`, `discharge_rate_idle`, `discharge_rate_active`, `no_overnight_charge`; charging-pad: `pad_disconnected`, `pad_unused`; system: `unexpected_reboot`) are purged from `alert_rules` on existing DBs by an idempotent `DELETE` in `migrationSQL`, which also scrubs them from per-channel `alert_types` allowlists.
