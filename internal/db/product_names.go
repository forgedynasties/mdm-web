...
func (d *DB) ProductNames(ctx context.Context) (map[string]string, error) {
	rows, err := d.pool.Query(ctx, `SELECT product, name FROM product_names`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (d *DB) SetProductName(ctx context.Context, key, name, by string) error {
	...
	if name == "" {
		_, err := d.pool.Exec(ctx, `DELETE FROM product_names WHERE product = $1`, key)
		return err
	}
...
func (d *DB) ProductOverview(ctx context.Context) ([]ProductRow, error) {
	rows, err := d.pool.Query(ctx, `
		SELECT COALESCE(NULLIF(d.product, ''), '`+prod.DefaultKey+`') AS p,
		       ...
		       COALESCE(MAX(pn.name), ''), COALESCE(MAX(pn.updated_by), '')
		FROM devices d
	...
	for rows.Next() {
		var r ProductRow
		if err := rows.Scan(&r.Key, &r.Devices, &r.Firmware, &r.DPC, &r.Lite, &r.Manufacturer, &r.Model, &r.Name, &r.UpdatedBy); err != nil {
			return nil, err
		}
		out = append(out, r)
	}
