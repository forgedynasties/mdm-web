...
// capPen mirrors db.capPenalty for the explainer: the same ceiling, without the
// rounding, so a reason's points and the score it explains cannot disagree.
func capPen(v, max float64) float64 {
	if v > max {
		return max
	}
	if v < 0 {
