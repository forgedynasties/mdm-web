...
var Changelog = []Entry{
	{
		...
		Changes: []string{
			...
			"Manage: kiosk lock is now a named, reusable policy — create one (like “Drive-Thru Lockdown”), assign it to a restaurant, group, or single device, and edit, duplicate, or delete it any time from its own page, instead of a one-off action you re-do from scratch each time. Kiosk mode also stays available on the Actions page for now, alongside the new Manage page.",
			"Alerts now has its own spot in the main navigation. Fleet Health was folded away — its score, verdict, and crash signals already live on the Overview page, so it's no longer a separate page.",
			"Much of the dashboard loads noticeably faster: Overview, Fleet, Actions, and the device page now run their several database lookups at the same time instead of one after another, static files are compressed, and clicking between the main pages feels instant thanks to quiet background preloading.",
			"The device page's graphs are lighter too: temperature and RAM only load once you open those tabs, the charging-pad chart no longer paints noisy single-sample blips, and a faulty pad now shows in orange instead of the same grey used for “disabled.”",
			"Every action button (Screenshot, Reboot, install, and more) now shows a clear loading state the moment you click it, so a fast action doesn't feel like nothing happened. Clicking Screenshot again while one is still pending now takes you straight to that in-progress command instead of just flashing a message.",
			"Fixed a bug where cancelling a queued command on an offline device didn't actually stop it from running once the device reconnected.",
		},
	},
	{
		Version: "1.40.0",
